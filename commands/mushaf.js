...
module.exports = class extends require('../structures/Command') {
	...
	async run(message, args) {
		...
		const message_collector = message.channel.createMessageCollector((m) => m.author.id === message.author.id && /^\d{1,3}$/.test(m.content), COLLECTOR_OPTIONS)
		const reaction_collector = m.createReactionCollector((reaction, user) => user.id === message.author.id && EMOJIS.includes(reaction.emoji.name), COLLECTOR_OPTIONS)
