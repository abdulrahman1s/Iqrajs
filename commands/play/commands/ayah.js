...
module.exports = async (message, args) => {
	...
	const reciter_list = await get_ayah_reciters();
...
function get_ayah_file(reciter, surah, ayah) {
	return `${reciter.ayah_url}/${String(surah).padStart(3, 0) + String(ayah).padStart(3, 0) + ".mp3"}`;
}